...
public class WitherSkeletonTrapGoal extends Goal {
    ...
    public WitherSkeletonTrapGoal(WitherSkeletonHorse horse) {
        this.horse = horse;
    }
    ...
    private AbstractHorse createHorse(DifficultyInstance p_30930_) {
        WitherSkeletonHorse skeletonhorse = SoullessSteeds.WITHER_SKELETON_HORSE.get().create(this.horse.level());
    ...
    @Nullable
    private WitherSkeleton createSkeleton(DifficultyInstance difficulty, AbstractHorse horse) {
        WitherSkeleton skeleton = EntityType.WITHER_SKELETON.create(horse.level());
        if (skeleton != null) {
